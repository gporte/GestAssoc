	...
	public class SaveInscriptionCommand : ICommand
	{
		...
		public void Execute(object parameter) {
			...
			try {
				if (this.IsValidForSaving(itemToSave, out errorsList)) {
					...
					NotificationHelper.WriteLog(Properties.Resources.Log_Ins_EnregistrementEffectue);
					new ShowViewCommand(ViewNames.ConsultationInscriptions.ToString()).Execute(null);
				}
				else {
					errorsList.Insert(0, Properties.Resources.Log_Ins_EnregistrementAnnule);
		...
		private bool IsValidForSaving(Inscription itemToSave, out List<string> errorsList) {
			...
			if (itemToSave.Adherent == null) {
				errorsList.Add(Properties.Resources.Err_Ins_AdherentObligatoire);
			}

			if (itemToSave.Groupe == null) {
				errorsList.Add(Properties.Resources.Err_Ins_GroupeObligatoire);
			}
			...
			if (itemExists) {
				errorsList.Add(Properties.Resources.Err_Ins_InscriptionExiste);
			}
		...
		private void SetStatus(Inscription itemToSave) {
			if (itemToSave.CertificatMedicalRemis && itemToSave.Cotisation > 0) {
				...
				NotificationHelper.WriteLog(Properties.Resources.Log_Ins_StatutValide);
			}
		}
	}
}
